...
| `libs/fm_fmc.*` | Totals, calibration, unit conversion, rate, decimal selection, formatting helpers. | `src/product/fmc/fmc_model.*`, `fmc_units.*`, `fmc_rate.*`, future volume and presentation helpers. | parcialmente migrado | Current FMC modules, specs, source. | `fmc_model_units_test` validates model/units/rate slices. | Visible ACM/TTL volume calculation, decimal policy, formatting, persistence interactions. | High | Core evidence for next `fmc_volume.*` and presentation slices. |
