...
| `FLOWMEET/fm_user.*` | User screens, TTL/RATE, ACM/RATE, print, Bluetooth, clock, navigation and actions. | Split into presentation, user-flow service, reporting, communication workflows. | comportamiento parcialmente documentado | `docs/specs/fmc/use_cases.yaml`; source. | Current LCD bringups and FMC model/rate harness cover only adjacent pieces. | Exact contract between values, screen modes, key events, and RTOS refresh. | High | Next likely evidence source for future presentation/user-flow slices. |
