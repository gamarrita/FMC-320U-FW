...
| `VOL_UNIT_BLANK` | active environment placeholder, not a physical unit | it exists as one selectable value in the product unit list; if the client unit is unsupported, the environment stays on this placeholder to avoid showing a wrong unit | maybe, but not as a normal physical volume unit in the core model | likely active-environment fallback plus presentation handling for unsupported/custom units | should the new design keep this as one enum value, or replace it with explicit unsupported-unit state? |
| `VOL_UNIT_BR` | active environment config with product-specific/custom semantics | legacy behavior suggests this can act as one client-specific/custom unit slot rather than a universally defined physical unit | maybe, if the product still needs one custom-unit slot | likely `fmc model` only if custom-unit support remains explicit; otherwise another unit-policy layer | is `BR` really a custom-unit placeholder in product terms, or should the new API rename this concept completely? |
...
| `VOL_UNIT_LT` | active environment config with legacy conversion-anchor role | physical unit offered to the user like any other; in the current legacy scheme it also acts as the table entry point used to turn `factor_cal` into `factor_k` | yes | fmc model | should the new design keep liters as the implicit conversion anchor, or make that role explicit and separate from the unit enum? |
...
| `rate` | runtime-derived | displayed/consumed flow value | yes | fmc model | should rate remain embedded in model or be split into runtime substate? |
...
| Item | Current classification | Why | Should survive refactor | Suggested destination | Open questions |
|---|---|---|---|---|---|
| `acm` | runtime-derived | accumulated total displayed to operator | yes | fmc model | none |
| `ttl` | runtime-derived | historical/privileged-reset total displayed to operator | yes | fmc model | reset policy now clarified as privileged, not user-level |
...
| `factor_cal` | strong calibration input, currently liters-anchored by legacy design | the deployed legacy scheme keeps one practical calibration path anchored in liters; this is acknowledged as a hack that worked and may later evolve into explicit calibration-unit support | yes | fmc model | keep the liters-anchored hack for first slice, or introduce explicit calibration-unit support now? |
| `factor_k` | derived helper / operative factor view with low-power purpose | precomputes `factor_cal * conversion_value` so runtime arithmetic is cheaper in an ultra-low-power embedded target; for conversion factor `1`, `factor_k == factor_cal` | yes as helper or query, not necessarily stored public truth | helper/query in model | should active K be visible to the user/config flow so unit changes become explicit? |
| `vol_unit` | active environment config | active shared volume unit | yes | fmc model | should `NONE/BLANK` remain part of model or move to presentation? |
