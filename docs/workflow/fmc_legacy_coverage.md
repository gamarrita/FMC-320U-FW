...
| `screens.SCREEN_STARTUP_ALL_SEGMENTS.display.backlight` | Replaced | `docs/product/fmc/behavior.md` | Phase 8 activates backlight when this view is successfully presented and uses the common fixed interval rather than coupling it to the full screen duration |
...
| `screens.SCREEN_TTL_RATE.display.pulse_indicator` | Replaced | `docs/product/fmc/behavior.md` | POINT now has a precise periodic-observation rule that applies transversely to every screen |
...
| `input_model.event_model.rule` | Replaced | `docs/product/fmc/behavior.md` | Key and action identity remain semantic, but product-main routes menu input directly to UI rather than retaining it in measurement runtime |
...
| `screens.SCREEN_ACM_RATE.display` | Replaced | `docs/product/fmc/user_interface.md` | Current contract fixes ACM on the upper row and reuses the accepted RATE representation plus transverse POINT |
...
| `screens.SCREEN_PRINTER.display` | Replaced | `docs/product/fmc/user_interface.md` | Phase 8 uses the static `PR` / `OFF` placeholder without progress or result states |
...
| `screens.SCREEN_BLUETOOTH_WINDOW.display` | Replaced | `docs/product/fmc/user_interface.md` | Phase 8 uses the static `LD` / `OFF` placeholder with no connection window or timeout |
...
| `screens.SCREEN_DATE_TIME.display` | Replaced | `docs/product/fmc/user_interface.md` | Phase 8 uses the static `DT` / `OFF` placeholder |
...
| `GLOBAL_USER_SCREEN_ALARMS.pulse_indicator` | Replaced | `docs/product/fmc/behavior.md` | POINT is driven by accepted periodic pulse observations with explicit toggle/off semantics |
